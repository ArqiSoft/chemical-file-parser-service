...
{
    public class ValidCdxTestFixture
    {
        ...
        public ValidCdxTestFixture(ChemicalFileParserTestHarness harness)
        {
            Bucket = UserId.ToString();
            BlobId = harness.UploadBlob(Bucket, "125_11Mos.cdx").Result;
            harness.ParseFile(Id, BlobId, Bucket, UserId, CorrelationId).Wait();
        }
    }

    [Collection("Chemical File Parser Test Harness")]
    public class ValidCdxTests : ChemicalFileParserTest, IClassFixture<ValidCdxTestFixture>
    {
        ...
        public ValidCdxTests(ChemicalFileParserTestHarness harness, ITestOutputHelper output, ValidCdxTestFixture initFixture) : base(harness, output)
        {
            Id = initFixture.Id;
        ...
        [Fact]
        public void CdxParsing_ValidCdxFile_ReceivedEventsShouldContainValidData()
        {
            var recordParsedEvents = Harness.GetRecordParsedEventsList(Id);
            recordParsedEvents.Should().HaveCount(3);
            ...
            fileParsedEvn.ParsedRecords.Should().Be(3);
            fileParsedEvn.ParsedRecords.Should().Be(3);
